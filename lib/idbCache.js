...
// Races a promise against a plain timeout — used wherever a live network
// call needs to fall back to cached data instead of hanging indefinitely
// on a connection that's present but not actually working (a common
// real-world state: connected to wifi with no upstream internet, which
// can take far longer to fail on its own than anyone should have to
// stare at a loading screen for). A genuinely fast failure — airplane
// mode, DNS refusing outright — hits the same rejection just as quickly
// as before this existed; this only helps the slow, ambiguous case.
