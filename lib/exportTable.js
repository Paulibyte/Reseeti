...
export async function exportPDF(report, filename, businessName) {
  ...
  // Same mobile-reliability fix as reports/page.js's downloadPDF() —
  // pdf.save(filename) depends on the browser's `download` attribute,
  // which mobile Safari largely ignores and installed PWAs on Android
  // handle inconsistently. Opening the generated PDF in a new tab
  // instead works everywhere: the browser's own PDF viewer takes over,
  // and the person can use its native Share/Download/Print options.
