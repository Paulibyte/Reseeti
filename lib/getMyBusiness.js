...
import { cacheGetMeta, cacheSetMeta, withTimeout } from './idbCache';

// How long a live membership check is allowed to hang before falling
// back to the last cached result — long enough that a normal, working
// connection never notices it, short enough that a real cold-open on a
// dead connection doesn't leave someone staring at a spinner.
const MEMBERSHIP_FETCH_TIMEOUT_MS = 4000;
...
export async function getMyBusiness(supabase) {
  ...
  if (!user) return { user: null, business: null, role: null, overrides: {}, businesses: [], fromCache: false };
  ...
  try {
    // Raced against a timeout — see MEMBERSHIP_FETCH_TIMEOUT_MS above.
    // This is the one genuinely network-dependent step in this whole
    // function; getSession() above never leaves local storage.
    ...
    list = data || [];
    cacheSetMeta(cacheKey, list); // Best-effort — see idbCache.js's own comment on this.
  } catch {
    // Live fetch failed or timed out — most likely a device that's
    // connected to *a* network but has no real internet, or is
    // genuinely offline from a cold app launch. Falls back to whatever
    // membership data was last cached for this exact person (cached on
    // every successful fetch above), so the app still opens with real,
    // if slightly stale, business/role data instead of an indefinite
    // spinner or a blank "no business found" screen. If this person has
    // never successfully loaded the app before on this device, there's
    // nothing to fall back to — list stays empty, same as today.
    list = (await cacheGetMeta(cacheKey)) || [];
    fromCache = true;
  }
  ...
  return {
    ...
    // True only when the line above came from cache, not a live fetch —
    // a new, purely additive field existing call sites can ignore
    // completely; dashboard/page.js uses it to skip further live calls
    // that would just time out the same way, and to show a small
    // "offline, last saved data" indicator.
    fromCache,
  };
}
